...
// LINUX SCHEDULER 
   ...
   // kprintf("current process counter %d \n", optr->counter);
    if (preempt > 0) {
     ...
     //         optr->counter, optr->goodness);
    }
    //kprintf("preempt  - %d\n",preempt);
    if (preempt <= 0) { // kprintf("preempt <0 %d\n", preempt);
      optr->goodness = 0;
    ...
    int trav;
    int max_goodness = 0;
    trav = q[rdyhead].qnext;

    // linear search for max goodness value process in ready queue
    while (q[trav].qnext != rdytail) {
      if (proctab[trav].goodness > max_goodness) {
        max_goodness = proctab[trav].goodness;
        next_proc = trav;
      }
