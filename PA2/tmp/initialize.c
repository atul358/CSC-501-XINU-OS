...
#define NOT_DEFINE -1
#define page_tab_base(i,j)	(i<<10)+j
...
queue_node frame_queue[NFRAMES];
...
extern int curr_frame, prev_frame, queue_head;
curr_frame = 0;
prev_frame = NOT_DEFINE; 
queue_head = NOT_DEFINE;
...
int j = 0;
void general_page_table(int proc_id)
{
	//kprintf("\nInitialize General Page Table START _----\n");
	int resp,i=0;
	int frm_num;
	do{
	    init_frm(i);
	...
	do{
		...
		int frame = page_table(proc_id);
		frame = update_frame(frame, proc_id, 1);
		pt_t *page_tb_ent;
		global_page_table[i] = (frame+FRAME0)*NBPG;
		int k;
		for(k=0; k <(NBPG/4); k++){
			pt_t *page_entery= get_base(frame)*k;
      			page_entery->pt_pres=SET;
      			page_entery->pt_global=SET;
			page_entery->pt_base= page_tab_base(i,k);
      			page_entery->pt_write=SET;
      			frm_tab[i].fr_refcnt++;
	   }i++;
     	}while(i < 4);
}

int page_table(int proc_id)
{
	int i,frm_num;
	pt_t *pe;
	get_frm(&frm_num);
...
int update_frame(int frm_num, int proc_id, int i)
{	
	frm_tab[frm_num].fr_status = FRM_MAPPED;
	frm_tab[frm_num].fr_dirty= NOT_SET;
	frm_tab[frm_num].fr_refcnt=NOT_SET;
	if(i ==2){
